...
1. On huggingface.co create a Space with SDK **Static**. Its address will be `https://<owner>-<name>.static.hf.space/` (lower case, underscores become hyphens). That is the address staff open, and the redirect URI for the app registration (section 1). Staff must open that address directly, not the Space's page on huggingface.co, which wraps it in a frame that Microsoft sign-in refuses.
2. Turn on two-factor sign-in for the Hugging Face account. Create a **fine-grained token with write access to that one Space only**, or better, set up a Trusted Publisher (GitHub Actions, this repository, branch `main`) on the Space so no token is stored anywhere.
3. In this GitHub repository, add the secret `HF_TOKEN` and the variable `HF_SPACE_ID` (Settings › Secrets and variables › Actions). The workflow in `.github/workflows/sync-to-space.yml` mirrors every push to `main` into the Space with Hugging Face's own sync action; the Space then builds it on Node 22 with `npm ci && npm run build` (the header at the top of `README.md` tells it how; the Space runs its own `npm install` first, so the install happens twice, which is harmless).
4. Commit `config.json` (from `public/config.example.json`) into `public/` on `main`. It holds ids and the shared mailbox address, no secrets.

**What is visible.** On a **public** Space everything is visible: the source, the built files and `config.json`. On a **protected** Space (paid plan, about £7 a month) the source is hidden, but the built files and `config.json` are still fetchable by anyone with the address, because the page has to be reachable. So treat the ids in `config.json` as non-secret (they are, by design of Microsoft's single-page-app sign-in) and rely on the app registration for access control: single tenant, assignment required, exact redirect addresses. A **private** Space does not work: colleagues without Hugging Face accounts get a 404. Static Spaces never sleep and are served from a CDN; a failed rebuild leaves the last good build live, so check the build log after each push (`hf spaces logs <space> --build`).

**What protects the sign-in even though a third party serves the code.** Real controls: single-tenant registration with assignment required; exact redirect addresses; least-privilege permissions (section 1); 24-hour sign-in tokens; Microsoft's sign-in logs (30 days on Business Premium) and the Purview audit log (180 days), both of which record this app's id against every mailbox and file access. Defence in depth only: the production build's Content-Security-Policy, which limits where the page can send data (a meta tag, because the host cannot set response headers; the host injects one small inline script of its own, which the policy blocks harmlessly with a console note); the lockfile build, which lets you download what the Space actually serves and compare it with a local build of the same commit. A Conditional Access rule requiring a company-managed device is worth having, but note it cannot be aimed at this app alone: it applies to Exchange and SharePoint for every client in the tenant, and needs Business Premium plus Intune.

**Residual risk, stated plainly.** If the host or the Hugging Face account were compromised, altered code could act as any signed-in user for the shared mailbox and the one quotes site, for as long as their session lasts (at most 24 hours), and would be attributed to this app in the audit logs. Nothing can be sent anywhere by the app itself, but code that replaces it is not bound by that. The only way to remove that risk entirely without Azure is to host the app inside SharePoint itself as a SharePoint Framework web part: Microsoft serves the code, staff open a SharePoint page, and the SharePoint data needs no extra consent. That is a rebuild of the shell (a different toolchain, older React) and needs the tenant app catalogue and an admin to approve the mailbox and Excel permissions; the screens and logic carry over. Keep it as the fallback if IT objects to Hugging Face.
