...
**Hosting and sign-in.** The app is a handful of static files, but SharePoint cannot host them directly: custom scripts are blocked on modern sites, and the supported route needs an admin-approved package. The independent review (section 13) made a point I agree with: the app's code holds the user's sign-in while it runs, so it should be served from somewhere the company controls. The first choice is therefore **Azure Static Web Apps** on the free tier in the company's own Azure subscription (in-tenant, Entra-authenticated, about £0); a free static page on Hugging Face (code only, never data) or Cloudflare Pages only if IT prefers not to open an Azure subscription. Either way the app needs an app registration in the Microsoft tenant so it can sign users in and read the shared mailbox and SharePoint on their behalf. Those are standard delegated permissions that do not need admin consent by default, though tenant settings may require it; ask IT whether the SharePoint permission can be scoped to the one quotes site. The person must already have access to the shared mailbox. Expect to sign in about once a day.
...
4. Where should the app's code live: the company's own Azure subscription (recommended by the review), a free public Hugging Face page (code only), or Cloudflare Pages? Does the company have, or want, an Azure subscription?
...
- **Code served from a third party while it holds the user's sign-in.** If the host or its account were compromised, swapped code could read the mailbox. Accepted: host the app in the company's own Azure subscription first (section 5), keep Hugging Face as the fallback, and ask IT to scope the SharePoint permission to the one site.
