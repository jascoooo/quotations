...
- **Code served from a third party while it holds the user's sign-in.** If the host or its account were compromised, swapped code could read the mailbox. The company has no Azure subscription, so the Hugging Face page stays; the protections in section 5 (mirrored public code, two-factor and a page-only token, exact redirect, a browser security policy that only allows Microsoft, device requirements from IT) are the answer.
