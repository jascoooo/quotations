...
describe('htmlToText', () => {
  it('turns an Outlook HTML body into something the matcher can read', () => {
    const html = '<html><head><style>p{color:red}</style></head><body><p>Attended 12/09/2026.</p><p>Garage door &amp; frame damaged.</p><br/>Thanks</body></html>';
...
describe('parseDrop', () => {
  ...
  it('reads a complete drop from the flow', () => {
    ...
    expect(e.attachments.map((a) => a.name)).toEqual(['front.jpg', 'damage.jpg']);
  });

  it('falls back to the file name when the flow sends no id', () => {
    const e = parseDrop(JSON.stringify({ subject: 'Hello' }), 'msg-000123.json');
  ...
  it('accepts an HTML body, because that is what the connector sends by default', () => {
    const e = parseDrop(JSON.stringify({ subject: 'x', body: '<p>Work order <b>SANC004958</b></p>' }), 'm.json');
    expect(e.bodyText).toBe('Work order SANC004958');
  });

  it('accepts a plain string sender and a single recipient', () => {
    const e = parseDrop(JSON.stringify({ subject: 'x', from: 'someone@example.com', to: 'me@example.com' }), 'm.json');
  ...
  it('uses now when the date is missing or unreadable, rather than producing an invalid one', () => {
    for (const when of [undefined, 'not a date']) {
      const e = parseDrop(JSON.stringify({ subject: 'x', receivedAt: when }), 'm.json');
  ...
  it('refuses a file that is not an email, with a reason naming the file', () => {
    ...
    expect(() => parseDrop(JSON.stringify({ subject: '  ' }), 'empty.json')).toThrow(/neither a subject nor a body/);
  });
