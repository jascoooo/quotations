...
export function parseDrop(json: string, fileName: string): Email {
  ...
  try {
    ...
    throw new Error(
      `${fileName} is not valid JSON. Check the flow's "Create file" content.`,
    );
  }
  if (!d || typeof d !== "object" || Array.isArray(d))
    throw new Error(`${fileName} does not contain an email.`);

  const base = fileName.replace(/\.json$/i, "");
  const id = String(d.id ?? base).trim() || base;
  const subject = String(d.subject ?? "").trim();
  const rawBody = d.bodyText ?? d.body ?? d.bodyPreview ?? "";
  const bodyText = /<[a-z][\s\S]*>/i.test(rawBody)
    ? htmlToText(rawBody)
    : String(rawBody);
  if (!subject && !bodyText)
    throw new Error(`${fileName} has neither a subject nor a body.`);

  const from =
    typeof d.from === "string"
      ? { name: d.from, address: d.from }
      : { name: d.from?.name ?? "", address: d.from?.address ?? "" };
  // The Outlook connector hands the recipients over as one semicolon-separated
  // string, so split it rather than treat the lot as a single address.
...
interface FsDirectoryHandleRW extends FsDirectoryHandle {
  getDirectoryHandle: (
    name: string,
    o?: { create?: boolean },
  ) => Promise<FsDirectoryHandleRW>;
  getFileHandle: (
    name: string,
    o?: { create?: boolean },
  ) => Promise<FsFileHandleRW>;
...
export async function listJson(
  ...
  for await (const entry of dir.values()) {
    ...
    out.push({
      name: entry.name,
      file: await (entry as FsFileHandle).getFile(),
    });
  }
  return out;
}
